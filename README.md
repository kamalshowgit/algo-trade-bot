...
- Forward paper/live runs can now auto-tune a more conservative trading profile from saved local `price_history*.csv` files before the session starts.
...
- `FORWARD_STRATEGY=strategy_4`
- `AUTO_TUNE_FROM_LOCAL_DATA=true`
- `MIN_LOCAL_TRAINING_FILES=3`
- `LOCAL_PRICE_HISTORY_PATTERNS=../price_history*.csv,price_history*.csv`

If your saved local sessions were generated by an older engine version, start with `AUTO_TUNE_FROM_LOCAL_DATA=false` for cleaner validation, then re-enable it after collecting fresh runs from the new engine.
...
If adaptive tuning is enabled and enough local `price_history` files exist, the bot will replay recent sessions, score a small set of safer entry windows and direction filters across all built-in strategies, and use the best profile for the next session.
